...
async def handle_voice_message(message: Message, state=None):
    ...
    if is_voice_processing(user_id):
        logger.info(f"⏳ Голос для {user_id} уже обрабатывается в другом месте, пропускаем")
        return
    
    set_voice_processing(user_id, True)
    logger.info(f"🎤 Установлен флаг обработки голоса для {user_id}")
...
"""
        ...
        # ✅ СНЯТИЕ ФЛАГА В ЛЮБОМ СЛУЧАЕ
        clear_voice_processing(user_id)
