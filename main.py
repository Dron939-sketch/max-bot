...
ВЕРСИЯ ДЛЯ MAX
ИСПРАВЛЕНО: правильный запуск FastAPI в отдельном процессе с собственным циклом событий
"""
...
# ============================================
# ФУНКЦИЯ ЗАПУСКА FASTAPI В ОТДЕЛЬНОМ ПРОЦЕССЕ - ГЛАВНОЕ ИСПРАВЛЕНИЕ
# ============================================

def run_fastapi_process():
    """Запускает FastAPI в отдельном процессе с собственным циклом событий"""
    ...
    # Применяем nest_asyncio к этому процессу
    nest_asyncio.apply(loop)
    
    # Запускаем uvicorn с правильным event loop
    config = uvicorn.Config(api_app, host="0.0.0.0", port=port, loop=loop)
...
def main():
    ...
    # ЗАПУСКАЕМ FASTAPI В ОТДЕЛЬНОМ ПРОЦЕССЕ (не в потоке!)
    # Это главное исправление для ошибки sniffio
...
if __name__ == "__main__":
    # Важно для multiprocessing на Render
