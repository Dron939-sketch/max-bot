...
ВЕРСИЯ ДЛЯ MAX
ЧИСТАЯ ВЕРСИЯ ДЛЯ PYTHON 3.11 (БЕЗ ПАТЧЕЙ)
"""
...
async def chat_action(request: Request):
    try:
        ...
        if action == "start_test":
            ...
            return JSONResponse({"success": True, "action": action, "data": profile_data})
        elif action == "show_thoughts":
            thoughts = await get_thought(user_id)
            return JSONResponse({"success": True, "action": action, "data": thoughts})
        elif action == "show_weekend":
            ideas = await get_ideas(user_id)
            return JSONResponse({"success": True, "action": action, "data": ideas})
        elif action == "ask_question":
            return JSONResponse({"success": True, "action": action, "data": {"message": "Задайте ваш вопрос"}})
...
async def get_test_question(user_id: int, stage: int, index: int):
    # ... (оставляем как есть, слишком длинный)
    pass
