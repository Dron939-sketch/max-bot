...
ИСПРАВЛЕНО: Добавлен патч для httpcore.AsyncShieldCancellation
ИСПРАВЛЕНО: Удалён вызов close_global_session (больше не используется)
"""
...
if sys.version_info >= (3, 14):
    ...
    # ===== ДОПОЛНИТЕЛЬНЫЙ ПАТЧ ДЛЯ HTTPCORE =====
    try:
        import httpcore
        from httpcore._synchronization import AsyncShieldCancellation
        
        original_async_shield_enter = AsyncShieldCancellation.__enter__
        
        def patched_async_shield_enter(self):
            """Безопасная версия __enter__ для AsyncShieldCancellation"""
            try:
                return original_async_shield_enter(self)
            except TypeError as e:
                if 'weak reference' in str(e):
                    return self
                raise
        
        AsyncShieldCancellation.__enter__ = patched_async_shield_enter
        print("✅ Применён патч для httpcore.AsyncShieldCancellation")
    except ImportError:
        print("ℹ️ httpcore не найден, пропускаем патч")
    except Exception as e:
        print(f"⚠️ Ошибка при патче httpcore: {e}")
    # =============================================
# =========================================================
...
async def test_weather_async(message: types.Message):
    """Асинхронное тестирование OpenWeather с использованием httpx"""
...
async def shutdown_handler():
    ...
    # ===== БЛОК С AIONTP УДАЛЕН (больше не используется) =====
